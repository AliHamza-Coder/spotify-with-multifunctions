...
async function get_songs() {
    ...
    for (const element of as) {
        if (element.href.endsWith("mp3")) {
            ...
            songs.push({title:name.replace(".mp3",""),source:"/Gaanay/"+path[path.length-1]})
        }
    }
    ganay=[...songs] // copy array or // ganay = JSON.parse(JSON.stringify(songs)); //copy array hacky method json
}
function display_songs(){
    let item = document.querySelector(".songs-list");
    var audio
    for (const song of ganay) {
        audio = new Audio(song.source)
        audio.addEventListener("loadedmetadata", () => {
        item.innerHTML += `<div class="song-view">
        <img src="SVG'S/music.svg"class="album-cover">
        <div class="song-details">
            <h3 class="song-title">${song.title}</h3>
            <h4 class="song-artist">It's Hamza😎</h4>
        </div>
